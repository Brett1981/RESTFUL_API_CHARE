...
namespace CHARE_REST_API_v1.Controllers
{
    public class MembersController : ApiController
    {
        ...
        // Custom GET: api/Members/5
        [ResponseType(typeof(Member))]
        public IHttpActionResult GetMember(string username)
        {
            Member member = db.Members.Where(x => x.username == username).FirstOrDefault();
            if (member == null)
            {
                return NotFound();
            }
        ...
        // Custom GET: api/Members/5
        [ResponseType(typeof(Member))]
        public IHttpActionResult GetMember(string username, string password)
        {
            Member member = db.Members.Where(x => x.username == username && x.password == password).FirstOrDefault();
            if (member == null)
            {
                return NotFound();
            }
